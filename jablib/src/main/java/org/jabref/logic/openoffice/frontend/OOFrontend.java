...
public class OOFrontend {
    ...
    public OOFrontend(XTextDocument doc) throws NoDocumentException, WrappedTargetException {
        ...
        Map<CitationGroupId, CitationGroup> citationGroups = readCitationGroupsFromDocument(this.backend, doc, citationGroupNames);
    ...
    private List<RangeSortable<CitationGroup>> createVisualSortInput(XTextDocument doc, boolean mapFootnotesToFootnoteMarks) throws NoDocumentException, WrappedTargetException {
        ...
        RangeSort.RangePartitions<RangeSortEntry<CitationGroup>> partitions = RangeSort.partitionAndSortRanges(sortables);
    ...
    public List<RangeForOverlapCheck<CitationGroupId>> citationRanges(XTextDocument doc) throws NoDocumentException, WrappedTargetException {
        ...
        List<RangeForOverlapCheck<CitationGroupId>> result = new ArrayList<>(citationGroups.numberOfCitationGroups());

        for (CitationGroup group : citationGroups.getCitationGroupsUnordered()) {
            XTextRange range = this.getMarkRange(doc, group).orElseThrow(IllegalStateException::new);
            String description = group.groupId.citationGroupIdAsString();
            result.add(new RangeForOverlapCheck<>(range, group.groupId, RangeForOverlapCheck.REFERENCE_MARK_KIND, description));
        }
        return result;
    }
    ...
    public List<RangeForOverlapCheck<CitationGroupId>> bibliographyRanges(XTextDocument doc) throws NoDocumentException, WrappedTargetException {
        ...
        if (range.isPresent()) {
            ...
            result.add(new RangeForOverlapCheck<>(range.get(), new CitationGroupId("bibliography"), RangeForOverlapCheck.BIBLIOGRAPHY_MARK_KIND, description));
        }
        return result;
    }
    ...
    public List<RangeForOverlapCheck<CitationGroupId>> viewCursorRanges(XTextDocument doc) {
        ...
        if (range.isPresent()) {
            ...
            result.add(new RangeForOverlapCheck<>(range.get(), new CitationGroupId("cursor"), RangeForOverlapCheck.CURSOR_MARK_KIND, description));
        }
        return result;
    }
    ...
    public List<RangeForOverlapCheck<CitationGroupId>> footnoteMarkRanges(List<RangeForOverlapCheck<CitationGroupId>> citationRanges) {
        ...
        for (List<RangeForOverlapCheck<CitationGroupId>> partition : partitions.getPartitions()) {
            ...
            result.add(new RangeForOverlapCheck<>(footnoteMarkRange.get(), citationRange.idWithinKind, RangeForOverlapCheck.FOOTNOTE_MARK_KIND, "FootnoteMark for " + citationRange.format()));
        }
        return result;
    }
    ...
    public OOVoidResult<JabRefException> checkRangeOverlapsWithCursor(XTextDocument doc, List<RangeForOverlapCheck<CitationGroupId>> userRanges, boolean requireSeparation) {
        try {
            ...
            return OOVoidResult.error(new JabRefException("Found overlapping or touching ranges", rangeOverlapsToMessage(overlaps)));
        } catch (NoDocumentException | WrappedTargetException e) {
    ...
    public OOVoidResult<JabRefException> checkRangeOverlaps(XTextDocument doc, List<RangeForOverlapCheck<CitationGroupId>> userRanges, boolean requireSeparation, int reportAtMost) {
        try {
            List<RangeForOverlapCheck<CitationGroupId>> citationRanges = citationRanges(doc);
            List<RangeForOverlapCheck<CitationGroupId>> ranges = new ArrayList<>();
            ranges.addAll(userRanges);
