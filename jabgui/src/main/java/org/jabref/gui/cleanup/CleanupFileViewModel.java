...
public class CleanupFileViewModel {
    ...
    public CleanupFileViewModel(CleanupPreferences preferences) {
        ...
        // The two "only ..." modifiers map 1:1 onto distinct CleanupSteps, so only one can be
        // active at a time. Selecting one therefore clears the other.
        ...
        renamePdfOnlyRelativeSelected.addListener((obs, oldVal, newVal) -> {
            if (newVal) {
                renamePdfOnlyPdfFilesSelected.set(false);
            }
        });
        renamePdfOnlyPdfFilesSelected.addListener((obs, oldVal, newVal) -> {
            if (newVal) {
                renamePdfOnlyRelativeSelected.set(false);
            }
        });
    ...
    public EnumSet<CleanupPreferences.CleanupStep> getSelectedJobs() {
        ...
        if (renamePdfSelected.get()) {
            ...
            } else if (renamePdfOnlyPdfFilesSelected.get()) {
        ...
        // Independent modifier: persisted regardless of which rename variant (if any) is selected.
